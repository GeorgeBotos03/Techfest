...
def _sanitize(features: Dict[str, Any]) -> Dict[str, Any]:
    """Nu trimitem IBAN-urile complet și tăiem câmpuri lungi pentru prompt."""
    f = dict(features)
...
def _fallback(features: Dict[str, Any], signals: Dict[str, Any]) -> Dict[str, Any]:
    """Explicație deterministică atunci când LLM nu e disponibil / dă eroare."""
    reasons: List[str] = []
...
def ai_explain(features: Dict[str, Any], signals: Dict[str, Any]) -> Dict[str, Any]:
    """
    ✅ Semnătura standard: (features, signals) -> dict cu
       {summary, key_reasons[], recommendations[]}
    Dacă OpenAI nu e configurat sau cade, returnăm fallback determinist.
    """
    ...
    user = (
        ...
        "Payment: " + json.dumps(_sanitize(features), ensure_ascii=False)
    )
